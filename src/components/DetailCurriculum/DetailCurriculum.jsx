...
const DetailCurriculum = ({ course }) => {
  ...
  const curriculumData = useMemo(() => CURRICULUM_DATA[course] ?? [], [course]);

  // Reset to open the first month element when course changes or component mounts
  useEffect(() => {
    if (curriculumData.length > 0) {
      setOpenIndexes([0]); // Open the first month
      setActiveIndex('0-0'); // Activate the first content of the first month
    }
  }, [course, curriculumData.length]);
                    ...
                    <Card.Header style={{ padding: 0 }} className="curriculum-card-header">
                      ...
                      >
                        <div className="curriculum-question-container">
                          <Text typography="heading6" className="curriculum-week">
                            {item.week}개월
                          </Text>
                          <Text typography="heading6" className="curriculum-title">
                            {item.title}
                          </Text>
                        </div>
                      ...
                      <Card.Body className="curriculum-card-body">
                        <div className="curriculum-content-detail">
                          {item.contents.map((content, contentIdx) => (
                            <div
                              key={contentIdx}
                              className={`curriculum-item-wrapper ${
                                activeIndex === `${idx}-${contentIdx}` ? 'active' : ''
                              }`}
                                ...
                                <div className="curriculum-hover-container">
                                  ...
                                  {typeof content === 'object' && (
                                    <Text
                                      ...
                                      className={`curriculum-content-description ${
                                        activeIndex === `${idx}-${contentIdx}` ? 'active' : ''
                                      }`}
                                    >
                                      {content.description}
                        ...
                        <div className="curriculum-content-detail">
                          {item.contents.map((content, contentIdx) => (
                            <div
                              key={contentIdx}
                              className={`curriculum-item-wrapper ${
                                activeIndex === `${idx + 4}-${contentIdx}` ? 'active' : ''
                              }`}
